...
const withHydratedGroups = (configuration, shapes) => {
  ...
  ); // leftward: more grouped, rightward: more atomic
  ...
  const shapesWithGroups = protoGroups.reduce(
    (shapes, g) => {
    ...
    shapes.map(s => {
      const directAncestor = s.ancestors[s.ancestors.length - 1];
      return { ...s, parent: directAncestor ? directAncestor.id : null };
    })
  );
...
const alignmentGuides = (configuration, shapes, guidedShapes, draggedShape) => {
  ...
  for (let i = 0; i < guidedShapes.length; i++) {
    ...
    for (let j = 0; j < shapes.length; j++) {
      ...
      if (!configuration.intraGroupManipulation && referenceShape.parent) continue; // for now, don't snap to grouped elements fixme could snap, but make sure transform is gloabl
      if (
